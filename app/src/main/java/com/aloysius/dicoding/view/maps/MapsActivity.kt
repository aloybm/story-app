...
import com.aloysius.dicoding.databinding.ActivityMapsBinding
...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun observeMapsData() {
        lifecycleScope.launchWhenStarted {
            mapsViewModel.mapsData.observe(this@MapsActivity) { result ->
                when (result) {
                    ...
                    is ResultState.Error -> {
                        showToast("Failed to load maps: ${result.error}")
                        showLoading(false)
                    }
    ...
    private fun showLoading(isLoading: Boolean) {
        if (isLoading) {
            Toast.makeText(this, "Loading maps data...", Toast.LENGTH_SHORT).show()
        } else {
